...
//    )
    ...
    override func viewDidLoad() {
        ...
        CleverTapVariables.shared.printAllVariables()
    ...
    private func setupChallengeSection() {
        challengesSectionView = ThreeImageCardSectionView(
            ...
            onSelect: { [weak self] index, item in
                ...
                switch index {
                case 0:CleverTapVariables.shared.nativeBanner(for: "Beauty") { [weak self] urls in
                        guard let self = self else { return }

                        DispatchQueue.main.async {
                            // ⭐ Must use explicit self inside closure
                            self.carousel.stopAutoScroll()

                            self.carousel.items = urls.map { CarouselItem.url($0) }

                            CleverTapVariables.shared.lastLoadedCarousel = urls

                            self.carousel.startAutoScroll()
                        }
                    }

                case 1:CleverTapVariables.shared.nativeBanner(for: "Sports") { [weak self] urls in
                        guard let self = self else { return }
                ...
                case 2: CleverTapVariables.shared.nativeBanner(for: "Clothes") { [weak self] urls in
                        guard let self = self else { return }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let saved = CleverTapVariables.shared.lastLoadedCarousel

        if !saved.isEmpty {
            self.carousel.items = saved.map { CarouselItem.url($0) }
            self.carousel.startAutoScroll()
        }
    }
